...
namespace Dossamer.FX
{
    ...
    public class TimedSquishing : MonoBehaviour
    {
        ...
        {
            [SerializeField] public float speed, min, max, duration;

            [SerializeField] public bool shouldLoop;
        ...
        {
            return DampingCoefficient(time, damping, duration) * 0.5f * (max - min) * Mathf.Sin(frequency * time) +
                   1f; // 0.5f * (max + min);
        }
        ...
        IEnumerator DoSquish(float duration)
        {
            ...
            while (_shouldLoop || Time.time - _squishStartTime < _duration)
            {
                ...
                if (!_shouldLoop)
                {
                    x = NetSquish(Time.time - _squishStartTime + 0.25f * Mathf.PI, _dampFactor, _duration,
                        1.2f * _minSquish, 0.8f * _maxSquish, _squishSpeed);
                    z = x;
                    y = NetSquish(Time.time - _squishStartTime, _dampFactor, _duration, _minSquish, _maxSquish,
                        _squishSpeed);
                }
                else
                {
                    x = BaseSinusoid(Time.time - _squishStartTime + 0.25f * Mathf.PI, 1.2f * _minSquish,
                        0.8f * _maxSquish, _squishSpeed);
                    z = x;
